...
export function OverviewSection({ data }: OverviewSectionProps) {
  ...
  return (
    <section className="bg-white px-4 py-6">
      <div className={`overflow-hidden rounded-xl bg-white ${hasOverviewText ? "border border-gray-200 shadow-sm" : ""}`}>
        <div className={`relative w-full bg-gray-200 ${hasOverviewText ? "aspect-[4/3]" : "aspect-[3/4] rounded-xl overflow-hidden"}`}>
          ...
          <div className="border-t border-gray-200">
            {(data.categoryLabel?.trim() || data.projectName?.trim()) && (
              <div className="flex border-b border-gray-100">
                <div className="w-28 shrink-0 bg-[#6a5acd] px-4 py-3">
                  <span className="whitespace-nowrap text-sm font-medium text-white">{data.categoryLabel}</span>
                </div>
            ...
            {data.details.map((detail, index) => {
              ...
              return (
                ...
                >
                  <div className="w-28 shrink-0 bg-[#6a5acd] px-4 py-3">
                    <span className="whitespace-nowrap text-sm font-medium text-white">{detail.label}</span>
                  </div>
                  <div className="flex-1 bg-white px-4 py-3">
