...
/**
 * Template B - 구조화된 부동산 랜딩 (보라 배너 + zigbang 로고 스타일)
 */
export function TemplateB({ data, projectId }: TemplateBProps) {
  const zigbangLogoURL = data.hero.zigbangLogoURL;
  let pointIndex = 1;
  const getPointLabel = () => `POINT ${pointIndex++}`;

  return (
    <div className="mx-auto min-h-screen w-full max-w-md overflow-hidden bg-gray-50">
      ...
      {data.factCheck?.enabled !== false && (
        <div id="preview-section-factcheck" className="scroll-mt-24">
          <FactCheckSection
            data={{ ...data.factCheck, pointLabel: getPointLabel() }}
      ...
      {data.location?.enabled !== false && (
        <div id="preview-section-location" className="scroll-mt-24">
          <LocationSection
            data={{ ...data.location, pointLabel: getPointLabel() }}
            zigbangLogoURL={zigbangLogoURL}
          />
        </div>
      )}
      {data.development?.enabled !== false && (
        <div id="preview-section-development" className="scroll-mt-24">
          <DevelopmentSection
            data={{ ...data.development, pointLabel: getPointLabel() }}
            zigbangLogoURL={zigbangLogoURL}
          />
        </div>
      )}
      {data.facilities?.enabled !== false && (
        <div id="preview-section-facilities" className="scroll-mt-24">
          <FacilitiesSection
            data={{ ...data.facilities, pointLabel: getPointLabel() }}
            zigbangLogoURL={zigbangLogoURL}
          />
        </div>
      )}
      {data.interior?.enabled !== false && (
        <div id="preview-section-interior" className="scroll-mt-24">
          <InteriorSection
            data={{ ...data.interior, pointLabel: getPointLabel() }}
            zigbangLogoURL={zigbangLogoURL}
          />
        </div>
      )}
      {data.floorPlan?.enabled !== false && (
        <div id="preview-section-floorplan" className="scroll-mt-24">
          <FloorPlanSection
            data={{ ...data.floorPlan, pointLabel: getPointLabel() }}
            zigbangLogoURL={zigbangLogoURL}
          />
        </div>
      )}
